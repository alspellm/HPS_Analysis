...
def radFrac(mass):
    ...
    radF = ( -1.14400e-01 + 1.15665e01*mass - 2.36420e02*pow(mass,2) + 2.17845e03*pow(mass,3) - 9.38035e3*pow(mass,4) + 1.53550e4*pow(mass,5) )
    return radF


def massRes(mass):
    #res = 3.40881 + 4.29919e-2*mass - 5.28684e-5*mass*mass
    res = 2.53854 + 3.30200e-3*mass + 1.20876e-4*mass*mass
