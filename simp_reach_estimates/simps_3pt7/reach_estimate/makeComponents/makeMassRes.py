...
for mass in invMasses:
    print(mass)
    apFilename = "/sdf/group/hps/users/alspellm/projects/simps_3pt7/simps/recon/tuple_ana/hadd_mass_%i_simp_recon_ana.root"%(mass)
    apFile = r.TFile(apFilename)
    apRecoMass_h = copy.deepcopy(apFile.Get("vtxana_kf_radMatchTight_2021_simpSR/vtxana_kf_radMatchTight_2021_simpSR_vtx_InvM_h") )
    apRecoMass_h.SetName("vtxana_radMatchTight_simpsSR_vtx_InvM%i_h"%(mass))
    if apRecoMass_h.GetEntries() < 1:
        continue
...
fitResult = massResScaled_ge.Fit('pol3',"ES")
fitFunc = massResScaled_ge.GetListOfFunctions().FindObject("pol3")
