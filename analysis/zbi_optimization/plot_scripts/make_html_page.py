...
parser.add_argument('--parentDir', type=str, dest="parent_dir", default =".")
options = parser.parse_args()

# Directory path
parent_dir = os.path.abspath(options.parent_dir)
plots_1d_dir='%s/%s'%(parent_dir, 'plots_1d')
# List all directories that will become html pages
html_page_names = [d for d in os.listdir(plots_1d_dir) if os.path.isdir(os.path.join(plots_1d_dir, d))]

master_dir = '%s/%s'%(parent_dir,'master_html_pages')

input_plots_1d_dir = parent_dir+'/plots_1d/' 
input_plots_2d_dir = parent_dir+'/plots_2d/'
input_bkg_models_dir = parent_dir+'/bkg_models/'
