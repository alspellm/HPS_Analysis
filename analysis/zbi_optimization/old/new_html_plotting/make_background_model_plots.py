...
for key, values in plots.items():
    ...
    for key, value in test_cuts.items():
        zbi = float(test_cuts_zbi[key])
        zcut = float(test_cuts_zcut[key])
        nsig = float(test_cuts_nsig[key])
        nbkg = float(test_cuts_nbkg[key])
        line = "Test Cut %s: %f [zbi] | zcut %f [mm]. Nsig=%f | Nbkg=%f"%(key, zbi, zcut, nsig, nbkg)
        text.append(line)
    ...
    for n, value in enumerate(values):
        ...
        utils.formatHisto(h,line_width = 2, line_color = colors[n])
    ...
    canvas = utils.plotTH1s(histos, canvas_name, LogY=True,ymin=0.01,ymax=11e6)
    legend = utils.makeLegend(canvas, histos)
    textbox = utils.makeLatexBox(canvas, text, text_x=0.1, text_size=0.02)
    canvas.SaveAs('%s/%s.png'%(plots_dir,canvas_name))

nsigs_g = utils.makeTGraph('Nsig', np.array(iterations,dtype=float), np.array(nsigs,dtype=float),title='Nsig',xlabel='Iteration', ylabel='Nsig and Nbkg', lineColor=colors[0], lineWidth=3)
nbkgs_g = utils.makeTGraph('Nbkg', np.array(iterations,dtype=float), np.array(nbkgs,dtype=float),title='Nbkg',xlabel='Iteration',ylabel='Nsig and Nbkg',lineColor=colors[1], lineWidth=3)
...
ymin = min(min(nsigs), min(nbkgs), min(zcuts))
ymax = max(max(nsigs),max(nbkgs), max(zcuts))
nsigs_g.GetYaxis().SetRangeUser(ymin,ymax)
#nsigs_g.GetYaxis().SetNdivisions(500)
graphs = [nsigs_g, nbkgs_g, zcuts_g]
canvas = utils.plotTGraphs(graphs, 'Iteration Results', LogY=True)
canvas.SetGrid()
canvas.SetGridx()
legend = utils.makeLegend(canvas, graphs)
canvas.SaveAs('%s/%s.png'%(plots_dir,'iteration_results'))
