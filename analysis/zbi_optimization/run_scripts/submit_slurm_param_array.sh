...
echo "mass: ${mass}"
outputfile=${outputfile}_signal_mass_${mass}_mev.root
...
hpstr "$config" -D -cutsJson "$iterCutsJson" -cutVariables "${cutVariablesArray[@]}" -newVariables "${newVariablesArray[@]}" -newVariableParams "${newVariableParamsArray[@]}" -o "$outputfile" -mass "$mass" -b "$nbkg" -z "$scan" > "${outputfile%.root}_log.txt"
