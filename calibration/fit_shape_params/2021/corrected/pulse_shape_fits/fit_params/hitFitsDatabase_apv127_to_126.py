    ...
    for feb in range(10):
        ...
        for hybrid in range(4):
            ...
            for channel in range(max_channel):
                ...
                svt_to_channel_map[svtid] = channel
            channel_index = channel_index + max_channel
    return svt_to_channel_map
...
###############################################################################################
svtid_ch_map = svtidToChannelMap2019()
...
for e in tree:
    ...
    if svtid > 4095 and (channel%128 == 127 or channel%128 == 0 or channel%8 == 7):
        badPulse = True
    ...
    dbtuple = (int(svtid), np.round(amp,2), round(t0,2), round(tau1,2), round(tau2,2), useneighbors,int(channel))
...
for svtid in range(24576):
    if svtid not in existingids.keys():
        channel = svtid_ch_map[svtid]
        print(channel)
        dbtuple = [int(svtid),0.0,0.0,0.0,0.0,True,channel]
        dbout.append(dbtuple)
...
with open(dboutF, 'w') as f:
    ...
    for i,e in enumerate(dbout):
        ...
        if e[5] == True:
            ...
            if len(e) < 7:
                print("issue with svtid ")
                continue
            channel = e[6]
            apv_chan = channel%128
            print("Found bad pulse on channel ", channel, " with apv_channel ", apv_chan)

            found_neighbor = False
            for ii in range(i-1,-1,-1):
                print("Checking left of channel: svtid ", dbout[ii][0], "apv_channel ", dbout[ii][6]%128) 
                nleft = nleft + 1
                if dbout[ii][5] == True:
                    continue
                #if apv_chan == 127 and dbout[ii][6]%128 == 126:
                #    continue
                else:
            ...
            for ii in range(i+1,i+127-apv_chan):
                #print("Checking right of channel: svtid ", dbout[ii][0]) 
                print("Checking right of channel: svtid ", dbout[ii][0], "apv_channel ", dbout[ii][6]%128) 
                nright = nright + 1
                if dbout[ii][5] == True:
                    continue
                if apv_chan == 127:
