...
def sortHistograms(histograms):
    ...
    for histo in histograms:
        n.append(histo.GetEntries())
    sortedN = sorted(n)
    for e in sortedN:
        idx = n.index(e)
        sortedHistograms.append(histograms[idx])
...
def buildLegend(canvas,x1=0.7, y1=0.24,x2=0.82,y2=0.88,textsize=0.018,separation=1.0,fill=0,border=0):
    #canvas.cd()
...
for histoname in histonames:
    ...
    for histo in histograms:
        ...
        hitcode = splitname[0]+splitname[1]
        col = colorsMap[inthitcode]
        formatHistogram(histo,linewidth=1,name=hitcode, fillcolor=col)
        tstack.Add(histo)
...
#Find ymax for stack
for tstack in plots:
    c = r.TCanvas("%s"%(tstack.GetName()), "%s"%(tstack.GetName()))
    c.cd()
    tstack.Draw("HIST")
...
for i,tstacks in enumerate(plotSames):
    ...
    c = r.TCanvas("%s"%(plotSamesNames[i]), "%s"%(plotSamesNames[i]))
    c.cd()
