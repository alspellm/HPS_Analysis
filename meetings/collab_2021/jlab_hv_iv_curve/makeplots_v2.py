    ...
    for ikey in currentMap:
        if vkey.replace("v_sens",'') in ikey:
            ...
            for i,v in enumerate(voltArray):
                ...
                indices = [index for index, element in enumerate(voltArray) if element == v and currentArray[index] > 0 and index < maxVoltIndex]
                #Smooth out spikes
                index = max(indices)
