...
#2016 displaced A' search
def radFrac_Ap(massGeV):
    radF = 0.1168 - 1.375*massGeV + 10.19*pow(massGeV,2) + 9.422*pow(massGeV,3) - 367.5*pow(massGeV,4) - 1023*pow(massGeV,5)
    return radF

def massRes_Ap(massMev):
    res = 0.9348 + 0.05442*massMev - 5.784e-4*pow(massMev,2) + 5.852e-6*pow(massMev,3) - 1.724e-8*pow(m,4)
    return res
